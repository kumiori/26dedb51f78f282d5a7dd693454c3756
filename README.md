...
The public app reads Notion when `NOTION_TOKEN` is available, or when Streamlit secrets contain either `NOTION_TOKEN`, `[notion].token`, or `[notion].api_key`. Without one of these, the app uses a session-local, initially empty registry so the UI remains testable without creating external records.
...
This flag is intentionally off by default. M1 has no public or anonymous write path.

## Sources of truth

- `config/takeover_trajectory.yaml` — read-only M1 timeline input (`trajectory-plan/v2`).
