...
def populate_test_registry(app: AppTest) -> None:
    app.session_state["takeover_entities"] = [
        {
            "id": "kumiori",
            "type": "person",
            "title": "kumiori",
            "label": "Person · Alien / initiator",
        ...
        },
        {
            ...
            "type": "person",
            "title": "Ave",
            "label": "Person · Alien / artist",
            "metadata": {"node_stage": "node_population"},
        },
    ]
    app.session_state["takeover_relations"] = [
        {
            "id": "registry-kumiori-ave",
            "source": "kumiori",
            "target": "ave",
...
def test_m1_initial_state_is_sparse(monkeypatch) -> None:
    ...
    footer = next(
        ...
        if 'class="takeover-footer"' in block.value
    )
...
def test_start_here_branches_before_identity_and_asks_only_mode_specific_fields(
    ...
    next(
        button for button in app.button if button.label == "Performance / dance"
    ).click().run(timeout=20)
    assert not app.exception
    assert {item.label for item in app.text_input} >= {"NAME", "ONE LINK"}
    assert any(
        item.label == "WHAT KIND OF MOVEMENT OR PRACTICE DO YOU BRING?"
        for item in app.text_area
    )


def test_start_here_authenticates_only_after_a_draft_then_persists_separate_records(
    monkeypatch,
) -> None:
    monkeypatch.delenv("NOTION_TOKEN", raising=False)
    app = AppTest.from_file("app.py")
    ...
    app.session_state["start_here_draft"] = {
        ...
        "display_name": "Signal",
        "payload": {
            "listening_link": "https://example.test/listen",
            "practice": "Live selection",
        },
    }
    app.run(timeout=20)
    ...
    next(
        button for button in app.button if button.label == "CREATE EMOJI IDENTITY"
    ).click().run(timeout=20)
    assert "PERSIST THIS CONTRIBUTION" in {button.label for button in app.button}
    next(
        button for button in app.button if button.label == "PERSIST THIS CONTRIBUTION"
    ).click().run(timeout=20)
    ...
    participant = next(
        iter(app.session_state["takeover_onboarding_participants"].values())
    )
    assert participant["role"] == "dj"
    assert participant["authority"] == "provisional"
    assert (
        app.session_state["takeover_onboarding_events"][0]["actor"] == participant["id"]
    )
...
def test_main_page_drop_link_opens_the_private_encrypted_drop(monkeypatch) -> None:
    ...
    assert "SELECT · ENCRYPT · SEND" in " ".join(
...
def test_landing_process_manifesto_and_entry_share_one_three_column_grid(
    monkeypatch,
) -> None:
    monkeypatch.delenv("NOTION_TOKEN", raising=False)
    monkeypatch.delenv("TAKEOVER_GA_MEASUREMENT_ID", raising=False)
    ...
    grid = next(
        ...
        if 'class="takeover-three-blocks"' in block.value
    )
    assert grid.count("<article") == 3
    assert grid.index('class="takeover-process"') < grid.index(
        'class="takeover-manifesto"'
    )
    assert grid.index('class="takeover-manifesto"') < grid.index(
        'class="takeover-entry"'
    )
...
def test_network_connections_and_state_portrait_open_dialogues(monkeypatch) -> None:
    ...
    app.query_params["relation"] = "registry-kumiori-ave"
    app.run(timeout=20)
    ...
    assert "2 active people" in corpus
    assert "0 latent known" in corpus
    assert "0 latent private" in corpus
    assert "0 unknown" in corpus
    assert "1 connections" in corpus
    assert "1.00 connectivity" in corpus
    assert "1 active relations" in corpus


def test_seeded_node_population_editor_requires_its_url_capability(
    ...
    app.run(timeout=20)
    assert not app.exception
    assert "This node is waiting for them." in " ".join(
...
def test_dialog_button_labels_override_the_global_dark_paragraph_colour() -> None:
    ...
    assert all(
        colour in CSS
        for colour in ("#f5f5f2", "#b9bbc2", "#858894", "#171a21", "#434753", "#315f78")
    )
...
def test_sidebar_has_a_visible_navigation_surface(monkeypatch) -> None:
    ...
    assert {
        ...
        "ORDER / ART",
        "VOICES",
    }.issubset({button.label for button in app.sidebar.button})
    assert not any(
        button.label.startswith(("EN ", "ET ", "RU ")) for button in app.sidebar.button
    )
    ...
    next(
        button for button in app.sidebar.button if button.label == "VOICES"
    ).click().run(timeout=20)
    sidebar_corpus = " ".join(block.value for block in app.sidebar.markdown)
...
def test_core_views_render_without_a_browser(monkeypatch) -> None:
    ...
    app = AppTest.from_file("app.py").run(timeout=20)
    next(button for button in app.button if button.label == "RESOURCES").click().run(
        timeout=20
    )
    assert not app.exception
    ...
    assert (
        "Ave · 2026-08-18 · willingness to invest; not committed funds"
        in resources_corpus
    )
    assert {button.label for button in app.main.button} >= {
        "BUY",
        "DONATE",
        "INVEST",
        "BET",
    ...
    assert len(app.sidebar.dataframe) == 3
    assert any(
        control.label == "SCALING FACTOR · s" for control in app.sidebar.number_input
    )
    assert any(
        expander.label == "RELEVANT DATASETS" for expander in app.sidebar.expander
    )
    sidebar_corpus = " ".join(block.value for block in app.sidebar.markdown)
...
def test_developer_add_node_flow(monkeypatch) -> None:
    ...
    next(button for button in app.button if button.label == "Add entity").click().run(
        timeout=20
    )
    assert not app.exception
...
def test_voices_exposes_weighted_corpus(monkeypatch) -> None:
    monkeypatch.delenv("NOTION_TOKEN", raising=False)
    app = AppTest.from_file("app.py").run(timeout=20)
    next(button for button in app.button if button.label == "VOICES").click().run(
        timeout=20
    )
    assert not app.exception
    corpus = " ".join(block.value for block in app.markdown)
    ...
    next(
        button for button in app.main.button if button.label == "ET · Eesti"
    ).click().run(timeout=20)
    assert not app.exception
    assert any("HÄÄLED" in block.value for block in app.markdown)


def test_translation_proposal_is_session_local_and_does_not_overwrite_corpus(
    monkeypatch,
) -> None:
    monkeypatch.delenv("NOTION_TOKEN", raising=False)
    app = AppTest.from_file("app.py").run(timeout=20)
    ...
    next(button for button in app.main.button if button.label == "+").click().run(
        timeout=20
    )
    assert not app.exception
...
def test_landing_contains_the_exact_take_over_sequence(monkeypatch) -> None:
    ...
    process_block = next(
        block.value
        for block in app.markdown
        if 'class="takeover-process"' in block.value
    )
    assert all(line in process_block for line in sequence)
    landing_action = next(
        block.value for block in app.markdown if 'class="takeover-entry"' in block.value
    )
    ...
    call_block = next(
        ...
        if 'class="sidebar-call"' in block.value
    )
